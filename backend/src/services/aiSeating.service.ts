...
export async function suggestSeatingService(weddingId: string, userId: string) {
  const wedding = await prisma.wedding.findFirst({
    where: { id: weddingId, ownerId: userId },
    select: { id: true },
  });
  if (!wedding) return { result: "not_found" as const };
  ...
  if (tables.length === 0) return { result: "no_tables" as const };
  if (guests.length === 0) return { result: "no_guests" as const };
  ...
  const clean = validateAssignments(parsed.assignments, tableSeats, validGuestIds);
