 ...
 */
function resolveJwtSecret(name, value) {
    if (!value || value === INSECURE_DEFAULT) {
        if (isProduction) {
            throw new Error(`[env] ${name} must be set to a secure value in production (no '${INSECURE_DEFAULT}' fallback).`);
        }
        console.warn(`[env] ${name} is unset or using the insecure default — set it in .env before deploying.`);
