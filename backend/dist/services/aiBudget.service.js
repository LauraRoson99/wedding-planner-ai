...
exports.suggestBudgetService = suggestBudgetService;
...
const SuggestedBudgetItemSchema = zod_1.z.object({
    name: zod_1.z.string().min(1),
    category: zod_1.z.enum(BUDGET_CATEGORIES),
    estimatedAmount: zod_1.z.number().min(0),
    notes: zod_1.z.string().nullable(),
});
const SuggestedProviderSchema = zod_1.z.object({
    name: zod_1.z.string().min(1),
    category: zod_1.z.enum(PROVIDER_CATEGORIES),
    notes: zod_1.z.string().nullable(),
});
const SuggestedBudgetSchema = zod_1.z.object({
    budgetItems: zod_1.z.array(SuggestedBudgetItemSchema),
    providers: zod_1.z.array(SuggestedProviderSchema),
});
...
async function suggestBudgetService(weddingId, userId, input) {
    ...
    const [budget, guestCount, existingItems, existingProviders] = await Promise.all([
        prisma_1.prisma.budget.findUnique({ where: { weddingId }, select: { totalAmount: true, currency: true } }),
        prisma_1.prisma.guest.count({ where: { weddingId, role: "PRIMARY" } }),
        prisma_1.prisma.budgetItem.findMany({ where: { weddingId }, select: { name: true }, take: 100 }),
        prisma_1.prisma.provider.findMany({ where: { weddingId }, select: { name: true, category: true }, take: 100 }),
    ]);
...
//# sourceMappingURL=aiBudget.service.js.map